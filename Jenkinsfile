pipeline {
    ...
    stages {
        ...
        stage ('Build') {
            steps {
                echo '${parameters.version} e ${parameters.next_version}'
        ...
        stage ('Analyse') {
            when{
                expression { GIT_BRANCH == 'origin/mater' || GIT_BRANCH == 'origin/develop' }
            }
            steps {
                echo 'Branch = ' + GIT_BRANCH
        ...
        stage ('Release') {
            ...
            steps {
                ...
                sh 'mvn release:prepare release:perform -Dresume=false -DdryRun=true'
